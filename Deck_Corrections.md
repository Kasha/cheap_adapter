...
## Corrections to the section above, after reading the original notebooks

Two things stated earlier need amending, and both make the story simpler.

**The protocol offset has a cause.** `G4_cross_lineage.ipynb` contains the
original hub construction, so the ~7.8 point gap is no longer a caveat to
apologise for. The original hub: FOUR spaces (DINOv2 small/base/large plus
bge-m3), each scaled by its mean column standard deviation, random 1,000-row
eval split, bge-m3 as the head's target, ridge alpha 1e-2. The reconstruction:
seven raw spaces, sequential split, SBERT targets, alpha 1.0. Five differences.
If asked why the rebuilt numbers differ, that is the answer — and knowing why
is a much better position than noting that.

**GPT-2 was never in the hub.** Earlier notes here described GPT-2 dominating
the concatenation's variance. True of the reconstruction, not of the original
four-space hub. Do not put it on a slide.

**One line worth adding to slide 4**, from the rebuild's own diagnostic:

> Why does a global linear map work at all when what encoders share is local?
> Each encoder's reach into the shared space: images 0.46–0.57, all four text
> spaces 0.02–0.05 — uniformly, collapsed and healthy alike. The hub is largely
> an image-side object. Transfer runs on a global component that is real, thin,
> and exactly why the hub works AND why it never reaches the ceiling.

**And on slide 7, under OPEN**, state the deferral rather than leaving it out:

> Benchmark B1 — pre-registered in Sources_and_Benchmarks, not run, deferred
> for time. Nothing in the project depends on it.

A pre-registered test with no result and no mention is the version an examiner
can make something of. With a stated reason, it is just a scoping decision.
